...
http.createServer(function(req,res){
    ...
    //图片响应
    if(pathName=='login_bg.jpg' && req.method == 'POST'){
        uploadFile(req,res);
    }
    ...
    else if(pathName.indexOf('css')>=0 && req.method == 'GET'){//是客户端要css的请求
        // 一个一个的把css都响应回去
    ...
    else if(pathName.indexOf('js')>=0 && req.method == 'GET'){//是客户端要js的请求
        // 一个一个的把js都响应回去
...
function addArticle(req,res){
    ...
    req.on("end",function(){
        ...
        if(dataStr!=''){
            var fileObj2={};
            fileObj2.chapterId = oldl1.length+1;
            fileObj2.chapterName=dataStr;
            fileObj2.views = 0;
            newl.push(fileObj2);
            oldl.push.apply(oldl1,newl);
            chapterList.push({
                "chapterId": fileObj2.chapterId,
                "chapterName": dataStr,
                "imgPath": "",
                "chapterDes": "",
                "chapterContent": "",
        ...
        oldl1=[];
        var listStr = JSON.stringify(oldl);
